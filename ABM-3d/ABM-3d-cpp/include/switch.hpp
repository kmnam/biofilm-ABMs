            ...
            if (j != group)
            {
                ...
                if (r > total && r < total + switch_prob)
                {
                    ...
                    for (int k = 0; k < n_attributes; ++k)
                    {
                        ...
                        cells(i, group_attributes[k]) = attribute; 
                    }
                    break;
                }
                total += switch_prob;
		         ...
		         boost::random::uniform_01<>& uniform_dist)
{
	...
	if (r > total)    // In this case, we are switching to some other group  
	{
		...
		if (k != group)
		{
			...
			// ... then sample the cell's new growth rate and attribute
			// values 
			...
			{
			    auto pair = std::make_pair(k, m);
			    T attribute = attribute_dists[pair](rng);
			    cells(idx_switch, group_attributes[m]) = attribute;
			}
			break;
		    }
		    total += switch_prob;
