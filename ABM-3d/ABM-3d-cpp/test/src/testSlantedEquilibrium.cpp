...
int main()
{
    ...
    // Define a vertical cell in which the cell is just touching the surface
    Array<double, 3, 1> r, n;
    const double theta = (45. / 180.) * boost::math::constants::pi<double>();
    ...
    for (int i = 0; i < 10000; ++i)
    {
        ...
        if (i % 1000 == 0)
        {
            ...
            Array<double, Dynamic, 6> dEdq_cell = cellCellForcesFromNeighbors<double>(
                cells, neighbors, R, Rcell, cell_cell_prefactors 
            );
            Array<double, Dynamic, 2> dEdq_surface_repulsion = cellSurfaceRepulsionForces<double>(
                cells, ss, R, E0, nz_threshold
            );
            Array<double, Dynamic, 2> dEdq_surface_adhesion = cellSurfaceAdhesionForces<double>(
