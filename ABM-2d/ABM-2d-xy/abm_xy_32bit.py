...
b = np.array([2/9, 1/3, 4/9, 0], dtype=np.float32)
bs = np.array([7/24, 1/4, 1/3, 1/8], dtype=np.float32)
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1]) as f:
        params = json.load(f, parse_float=np.float32)

    # Define required input parameters
    R = params['R']                         # Outer cell radius (including EPS)
    L0 = params['L0']                       # Initial cell length (excluding caps)
    Ldiv = np.float32(2 * L0 + 2 * R)       # Division length (excluding caps)
    ...
    neighbor_threshold = np.float32(2 * (2 * R + L0))          # Radius for neighboring cells
    t_final = params['t_final']             # Final timepoint
    n_cells = params['n_cells']             # Desired number of cells

    # Surface contact area density
    surface_contact_density = np.float32((sigma0 * R * R / (4 * E0)) ** (1 / 3))
    ...
    while t < t_final and cells.shape[0] < n_cells:
        ...
        if i % iter_update_stepsize == 0:
            ...
            while max_error > 1e-8 and j < max_tries:
                dt *= np.float32((1e-8 / max_error) ** (1 / (error_order + 1)))
                cells_new, errors = step_RK_adaptive_from_neighbors(
                    A, b, bs, c, cells, neighbors, dt, R, Rcell, E0, Ecell,
                    surface_contact_density, rng, noise_scale
                )
            ...
            if max_error < 1e-8:
                dt = np.float32(
                    np.min([dt * ((1e-8 / max_error) ** (1 / (error_order + 1))), 1e-4])
                )
        cells = cells_new
