"""
Implementations of cell-cell and cell-surface interaction forces with 32-bit
...
def composite_viscosity_force_prefactors(cells, R, surface_contact_density):
    ...
    """
    KL = np.zeros((cells.shape[0], 2), dtype=np.float32)
    composite_drag = cells[:, 7] + cells[:, 8] * surface_contact_density / R
    KL[:, 0] = cells[:, 4] * composite_drag
    KL[:, 1] = np.power(cells[:, 4], 3) * composite_drag / 12
...
def cell_cell_distance(r1, n1, l1, r2, n2, l2):
    ...
    of the two cells. 
    """
    half_l1 = np.float32(l1 / 2)
    half_l2 = np.float32(l2 / 2)
...
def get_cell_neighbors(cells, neighbor_threshold, R, Ldiv):
    ...
    neighbors = np.zeros((n * (n - 1) // 2, 6), dtype=np.float32)
    idx = 0
...
def cell_cell_forces(cells, neighbor_threshold, R, Rcell, Ldiv, E0, Ecell):
    ...
    dEdq = np.zeros((n, 4), dtype=np.float32)
...
def get_velocities(cells, neighbor_threshold, R, Rcell, Ldiv, E0, Ecell,
    ...
    # 
    velocities = np.zeros((cells.shape[0], 4), dtype=np.float32)
    prefactors = composite_viscosity_force_prefactors(cells, R, surface_contact_density)
    K = prefactors[:, 0].copy().reshape((-1, 1))   # Copy essential for use with numba
    L = prefactors[:, 1].copy().reshape((-1, 1))   # Copy essential for use with numba
    dEdq = cell_cell_forces(cells, neighbor_threshold, R, Rcell, Ldiv, E0, Ecell)
    mult = (cells[:, 2] * dEdq[:, 2] + cells[:, 3] * dEdq[:, 3]).reshape((-1, 1))
    velocities[:, :2] = -dEdq[:, :2] / K
    velocities[:, 2:4] = (-dEdq[:, 2:4] - mult * cells[:, 2:4]) / L
...
def step_RK(A, b, c, cells, neighbor_threshold, dt, R, Rcell, Ldiv, E0, Ecell, 
    ...
    s = b.size
    velocities = np.zeros((cells.shape[0], 4, s), dtype=np.float32)
    velocities[:, :, 0] = get_velocities(
        cells, neighbor_threshold, R, Rcell, Ldiv, E0, Ecell, surface_contact_density
    )
    for i in range(1, s):
        multipliers = np.array((cells.shape[0], 4), dtype=np.float32)
        for j in range(i):
    ...
    velocities_final = np.zeros((cells.shape[0], 4), dtype=np.float32)
    for i in range(s):
...
def step_RK_adaptive(A, b, bs, c, cells, neighbor_threshold, dt, R, Rcell, Ldiv,
    ...
    # Compute Runge-Kutta update from computed velocities
    velocities_final1 = np.zeros((cells.shape[0], 4), dtype=np.float32)
    velocities_final2 = np.zeros((cells.shape[0], 4), dtype=np.float32)
    for i in range(s):
...
def step_RK_from_neighbors(A, b, c, cells, neighbors, dt, R, Rcell, E0, Ecell, 
    ...
    for i in range(1, s):
        multipliers = np.zeros((cells.shape[0], 4), dtype=np.float32)
        for j in range(i):
            multipliers += A[i, j] * velocities[:, :, j]
        cells_i = cells.copy()
        cells_i[:, :4] += multipliers * dt
