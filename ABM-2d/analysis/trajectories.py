...
########################################################################
def plot_trajectory_frame(trajectory, idx, boundary, R, plot_prev=True, res=50,
                          image_scale=2, position=None, show=False):
    """
    Plot the given frame of the given trajectory. 
    ...
    with multiprocessing.Pool(1) as pool:
        ...
        if plot_prev:
            ...
            for i in range(points.shape[0] - 1):
                pl.add_mesh(
                    pv.Line(pointa=points[i, :], pointb=points[i+1, :]),
            ...
            for i in range(points.shape[0] - 1):
                ...
                pl.add_mesh(
                    pv.Sphere(radius=0.8 * R, center=points[i, :]),
                    color=color
                )
        ...
        # spherocylinder
        r = np.concatenate((trajectory[idx, _colseq_r2], [0]))
        n = np.concatenate((trajectory[idx, _colseq_n2], [0]))
        ...
        cap1_center = r - 0.5 * l * n
        cap2_center = r + 0.5 * l * n
        cap1 = pv.Sphere(
            center=cap1_center,
            direction=n,
            radius=R,
        ...
        boundary = np.hstack((boundary, np.zeros(boundary.shape[0]).reshape(-1, 1)))
