...
##########################################################################
def plot_graph_2d(cells, graph, components, component_sizes, ax):
    """
...
if __name__ == '__main__':
    ...
    #
    # First pick out the desired number of frames
    timepoints = np.array(timepoints)
    t_mesh = np.linspace(timepoints[0], timepoints[-1], nframes_total)
    plot_filenames_subset = []
    for t in t_mesh:
        idx = np.argmin(np.abs(timepoints - t))
        plot_filenames_subset.append(plot_filenames[idx])

    # Then stitch the frames together 
    width = None
    height = None
    with Image.open(plot_filenames_subset[0]) as image:
        width, height = image.size
    fourcc = cv2.VideoWriter_fourcc('M', 'J', 'P', 'G')
    outfilename = os.path.join(
        os.path.dirname(plot_filenames_subset[0]),
        os.path.basename(cells_filenames[-1])[:-4] + '_combined.avi'
    )
    video = cv2.VideoWriter(
        outfilename, fourcc, 20, (width, height), isColor=True
    )
    for plot_filename in plot_filenames_subset:
