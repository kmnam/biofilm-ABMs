...
#include "../include/simulationWithRamp.hpp"
...
int main(int argc, char** argv)
{
    ...
    const T postswitch_slope = static_cast<T>(json_data["postswitch_slope"].as_double());  
    ...
    const int __colidx_targeteta1 = __ncols_required; 
    std::vector<int> group_attributes { __colidx_eta1 };
    std::vector<int> group_targetcols { __colidx_targeteta1 };
    ...
    cells << 0, 0, 0, 1, 0, 0, 0, 0, 0, L0, L0 / 2, 0, growth_mean, eta_ambient,
             eta_mean1, eta_mean1, 1, eta_mean1;
    ...
    runSimulationAdaptiveLagrangian<T>(
        ...
        group_attributes, group_targetcols, growth_means, growth_stds,
        attribute_means, attribute_stds, SwitchMode::MARKOV, switch_rates,
        postswitch_slope, daughter_length_std, daughter_angle_bound,
        truncate_surface_friction, surface_coulomb_coeff, max_noise, adhesion_mode,
        adhesion_map, adhesion_params, confine_mode, confine_params, growth_void_mode,
        growth_void_params
    ); 
