 ...
 * @param group_targetcols Indices of target values for the attributes that 
 ...
 * @param postswitch_slope Slope in switching attribute after switching event. 
...
std::pair<Array<T, Dynamic, Dynamic>, std::vector<int> >
    runSimulationAdaptiveLagrangian(const Ref<const Array<T, Dynamic, Dynamic> >& cells_init,
                                    ...
                                    std::vector<int>& group_targetcols, 
                                    ...
                                    const bool track_poles = false)
{
    ...
    for (int i = 0; i < n_attributes; ++i)
    {
        ...
        ss.str(std::string()); 
        ss << "group_targetcol" << i + 1; 
        params[ss.str()] = std::to_string(group_targetcols[i]); 
    }
    for (int i = 0; i < n_groups; ++i)
    {
        std::stringstream ss; 
    ...
    params["postswitch_slope"] = floatToString<T>(postswitch_slope); 
    ...
    for (int j = 0; j < n_attributes; ++j)
    {
        if (group_targetcols[j] >= __ncols_required)
            write_other_cols[group_targetcols[j]] = 1;
    }
    if (write)
    {
    ...
    while (!terminate(n, iter))
    {
        ...
        if (to_divide.sum() > 0)
        {
            ...
            if (switch_mode == SwitchMode::INHERIT)
            {
                switchGroupsInherit<T>(
                    cells, daughter_pairs, group_targetcols, n_groups,
        ...
        if (switch_mode == SwitchMode::MARKOV)
        {
            ...
            switchGroupsMarkov<T>(
                cells, group_targetcols, n_groups, dt, switch_rates, growth_dists,
            ...
            */
            // Change cell-surface friction according to the ramp 
            for (int i = 0; i < n; ++i)
            {
                for (int j = 0; j < n_attributes; ++j)
                {
                    int attribute = group_attributes[j]; 
                    int targetcol = group_targetcols[j]; 
                    if (cells(i, attribute) < cells(i, targetcol))
                    {
                        cells(i, attribute) = min(
                            cells(i, attribute) + dt * postswitch_slope,
                            cells(i, targetcol)
                        );
                    }
                    else if (cells(i, attribute) > cells(i, targetcol))
                    {
                        cells(i, attribute) = max(
                            cells(i, attribute) - dt * postswitch_slope,
                            cells(i, targetcol)
                        );
                    }
