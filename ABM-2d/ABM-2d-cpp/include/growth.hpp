                                       ...
                                       std::function<T(boost::random::mt19937&)>& daughter_angle_dist)
{
    ...
    if (n_divide > 0)
    {
        ...
        for (auto it = idx_divide.begin(); it != idx_divide.end(); ++it)
        {
            int group = static_cast<int>(cells_total(*it, 9)); 
            cells_total(*it, 6) = growth_dists[group - 1](rng);
        } 
        for (int i = 0; i < n_divide; ++i)
        {
            int group = static_cast<int>(new_cells(i, 9));
            new_cells(i, 6) = growth_dists[group - 1](rng);
        }
