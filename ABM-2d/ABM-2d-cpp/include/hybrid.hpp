/**
 * Implementations of hybrid Hertz-Kihara potential-based cell-cell interaction
 * forces.
 *
...
#define HYBRID_BIOFILM_CELL_CELL_POTENTIAL_FORCES_HPP
...
/**
 * Compute the derivatives of the hybrid Hertz-Kihara cell-cell potential for
 * each cell with respect to the cell's position and orientation coordinates.
 *
                                          ...
                                          const Ref<const Array<int, Dynamic, 1> >& repulsive_only)
{
    ...
    // Compute cell-cell overlap for every pair of neighboring cells
    ...
    for (int k = 0; k < neighbors.rows(); ++k)
    {
        ...
        if (overlap > 0 && overlap < R - Rcell)
        {
            ...
            {
                if (dist > dmin)
                    prefactor_attr = prefactor_12 / std::pow(dist, 13) - prefactor_6 / std::pow(dist, 7);
            }
        }
        ...
        Array<T, 2, 1> vij = (prefactor_repel + prefactor_attr) * dir_ij;
 ...
 * been pre-computed.
 *
 * This function uses forces calculated with the hybrid Hertz-Kihara potential. 
 *
 * @param cells Existing population of cells. 
 * @param neighbors Array specifying pairs of neighboring cells in the
 *                  population. 
